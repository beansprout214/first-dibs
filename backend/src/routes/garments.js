...
router.post('/:id/claim', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO claims (garment_id, claimant_name)
       VALUES ($1, $2)
       RETURNING claimant_name, claimed_at`,
      [id, claimantName.trim()]
    );
...
router.post('/:id/unclaim', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `DELETE FROM claims
       WHERE garment_id = $1 AND claimant_name = $2
       RETURNING id`,
      [id, (claimantName || '').trim()]
    );

    if (result.rowCount === 0) {
      return res.status(404).json({ error: 'No matching claim found to remove.' });
    }
    res.status(204).send();
  } catch (err) {
    console.error(err);
